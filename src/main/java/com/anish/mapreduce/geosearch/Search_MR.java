...
import com.anish.mapreduce.geosearch.Coordinate;

/**
 * The Search class. This has the Mapper class which joins the available (for
 * which data is known) coordinates data set with the search queries coordinates
 * data set. The main function submits the job.
 * 
 * @author anish
 *
 */
public class Search_MR {
	 ...
	 * coordinates. Then calls the mapper to map each record to one of the
	 * available coordinates.
	 * 
	 * @author anish
	 *
	 */
	static class GeoSearchMapper extends Mapper<LongWritable, Text, Text, Text> {
		private ArrayList<Coordinate> idx = new ArrayList<Coordinate>();

		/**
		 * Reads the file at given Path and populates the list idx with
		 * Coordinate objects.
		 * 
		 * @param path
		 * @throws IOException
		...
		/**
		 * Calls readIndex to cache the index
		 */
		...
		public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			while (low <= hi) {
				if (searchLat - latLimit > idx.get(mid).getLat())
					...
					hi = mid - 1;
				else if (Math.abs(idx.get(mid).getLat() - searchLat) < latLimit)
				// When mid is almost correct
				{
		 ...
		 * @param inputlat2
		 * @param inputlong2
		 * @return Distance between point lat1,lon1 and lat2,lon2
		 */
